...
This script executes the training process for CatBoost models.
Functionality includes:
1. Training on cumulative datasets (train_k.csv).
2. Support for GPU acceleration with automatic CPU fallback.
3. Performance evaluation (R2, RMSE, MAE).
4. Inference benchmarking.
5. Saving the best model based on R2 score.
"""
...
RAW_DIR = Path(config.raw_metrics_cb_dir)
RAW_DIR.mkdir(parents=True, exist_ok=True)

MODELS_ROOT = Path(getattr(config, "trained_models_cb_dir", str(ROOT_DIR / "results/02_trained_models/cb")))
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(RAW_DIR / "train_cb_log.log", encoding="utf-8")
    ]
)
logger = logging.getLogger("CB_TRAIN")
...
def _load_best_hyperparams(target: str) -> Dict[str, Any]:
    ...
    hp_dir = Path(config.hyperparameters_cb_dir)
    for p in [*hp_dir.glob(f"*{target}*.json"), *hp_dir.glob("*.json")]:
        ...
        if isinstance(obj, dict):
            ...
            if {"iterations", "depth", "learning_rate"} & set(obj.keys()):
                return dict(obj)

    raise FileNotFoundError(f"CatBoost hyperparameters not found for target: {target} in {hp_dir}")


def _coerce_numbers(d: Dict[str, Any]) -> None:
    """Ensures numeric parameters are correctly typed."""
    float_keys = {"learning_rate", "l2_leaf_reg", "subsample", "bagging_temperature", "random_strength", "rsm"}
    int_keys = {"iterations", "depth", "min_data_in_leaf", "max_bin", "border_count"}
...
def _ensure_gpu_params(params: Dict[str, Any]) -> Dict[str, Any]:
    """Configures parameters for GPU training with appropriate fallback settings."""
    p = dict(params)
    _coerce_numbers(p)
    ...
    subsample = p.get("subsample", None)
    has_sub = False
    try:
        has_sub = (subsample is not None and float(subsample) < 1.0)
    except Exception:
        has_sub = False

    bt = str(p.get("bootstrap_type", "") or "").strip().lower()
    ...
    loss = str(p.get("loss_function", "RMSE")).lower()
    pairwise_losses = {"pairwise", "pairlogit", "pairlogitpairwise", "yeti_rank", "yetrank", "queryrmse"}
    if p.get("task_type", "GPU").upper() == "GPU":
        if loss not in pairwise_losses:
            p.pop("rsm", None)

    for bad in ("langevin", "diffusion_temperature"):
        p.pop(bad, None)

    p.setdefault("iterations", 1000)
...
def measure_inference_time_three_files_avg(model, feature_cols, features_dir: Path, seed: int = 42):
    ...
    chosen = rnd.sample(files, k=min(3, len(files)))
    times, used = [], []
...
def _purge_old_best(dir_path: Path, target: str):
    ...
    for p in dir_path.glob(f"CB_*_{target}.cbm"):
        try:
            p.unlink()
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    for target in PREDICTED_PARAMETERS:
        ...
        raw_params = _load_best_hyperparams(target)
        params = _ensure_gpu_params(raw_params)

        metrics_by_size: Dict[str, Dict[str, float]] = {}
        best_info = dict(r2=-1e9, k=None, rmse=None, mae=None,
                         train_time=None, pred_time=None,
                         model_size=None, train_rows=None, model_path=None)
        ...
        for k, train_path in train_list:
            ...
            # Training with CPU Fallback
            try:
                model = CatBoostRegressor(**params)
                t0 = time.perf_counter()
                model.fit(train_pool, verbose=False)
                train_time = time.perf_counter() - t0
                used_task_type = model.get_param("task_type") or "GPU"
            except Exception as e:
                logger.warning(f"[{target}|k={k}] CatBoost GPU failed ({e}); falling back to CPU.")
                params_cpu = dict(params)
                params_cpu["task_type"] = "CPU"

                if str(params_cpu.get("bootstrap_type", "")).lower() == "bayesian":
                    params_cpu.pop("subsample", None)
                else:
                    params_cpu.pop("bagging_temperature", None)
            ...
            # Measure Model Size
            tmp_model_path = RAW_DIR / f"_tmp_cb_{target}_k{k}.cbm"
            ...
            logger.info(
                f"[CatBoost][target={target}][k={k}] "
                f"R2={r2_test:.6f} RMSE={rmse_test:.6f} MAE={mae_test:.6f} | "
                f"Train Time={train_time:.2f}s Pred Time={avg_pred_time:.4f}s Size={size_mb:.2f}MB | "
                f"Task Type={used_task_type} Bootstrap={model.get_param('bootstrap_type')}"
            )

            # Update Best Model
            if r2_test > best_info["r2"]:
                _purge_old_best(MODELS_ROOT, target)
                best_filename = f"CB_{k}_{target}.cbm"
                tmp_save = MODELS_ROOT / f".tmp_{best_filename}"
                final_best_path = MODELS_ROOT / best_filename

                model.save_model(tmp_save)
                ...
                best_info.update(dict(
                    ...
                    model_size=float(size_mb), train_rows=int(len(X_tr)), model_path=str(final_best_path)
                ))
        ...
        out = {
            ...
            "environment": {"task_type": used_task_type}
        }
