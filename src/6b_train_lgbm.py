...
This script executes the training process for LightGBM models.
Functionality includes:
1. Training on cumulative datasets (train_k.csv).
2. GPU acceleration support with automatic CPU fallback.
...
RAW_DIR = Path(config.raw_metrics_gb_dir)
RAW_DIR.mkdir(parents=True, exist_ok=True)

MODELS_ROOT = Path(getattr(config, "trained_models_gb_dir", str(ROOT_DIR / "results/02_trained_models/gb")))
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(RAW_DIR / "train_gb_log.log", encoding="utf-8")
    ]
)
...
def _load_best_hyperparams(target: str) -> Dict[str, Any]:
    ...
    hp_dir = Path(config.hyperparameters_gb_dir)
    for p in [*hp_dir.glob(f"*{target}*.json"), *hp_dir.glob("*.json")]:
        ...
        if isinstance(obj, dict):
            ...
            if {"n_estimators", "max_depth", "learning_rate"} & set(obj.keys()):
                return dict(obj)

    raise FileNotFoundError(f"LGBM hyperparameters not found for target: {target} in {hp_dir}")
...
def _ensure_gpu_params(params: Dict[str, Any]) -> Dict[str, Any]:
    """Ensures default parameters for GPU execution are set."""
    p = dict(params)
...
def _purge_old_best(dir_path: Path, target: str):
    ...
    for p in dir_path.glob(f"GB_*_{target}.pkl"):
        try:
            p.unlink()
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    for target in PREDICTED_PARAMETERS:
        ...
        params = _load_best_hyperparams(target)
        ...
        for k, train_path in train_list:
            ...
            try:
                ...
                used_device = model.get_params().get("device", "gpu")
            except Exception as e:
                logger.warning(f"[{target}|k={k}] LGBM GPU failed ({e}); falling back to CPU.")
            ...
            # Measure Model Size
            tmp_model_path = RAW_DIR / f"_tmp_gb_{target}_k{k}.pkl"
            ...
            logger.info(
                f"[LGBM][target={target}][k={k}] "
                f"R2={r2_test:.6f} RMSE={rmse_test:.6f} MAE={mae_test:.6f} | "
                f"Train Time={train_time:.2f}s Pred Time={avg_pred_time:.4f}s Size={size_mb:.2f}MB | Device={used_device}"
            )

            # Update Best Model
            if r2_test > best_info["r2"]:
                _purge_old_best(MODELS_ROOT, target)
                best_filename = f"GB_{k}_{target}.pkl"
        ...
        out = {
            ...
            "environment": {"device": used_device}
        }
